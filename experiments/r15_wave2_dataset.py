"""Merge and audit R15 internal Wave 2 structural confirmatory shards."""
...
WAVE2_SPECS: tuple[tuple[str, dict[str, str]], ...] = (
    ("HIST-01", {"mode": "natural-crossing"}),
    ("HIST-02", {}),
    ("HIST-03", {}),
    ("HIST-05", {}),
    ("HIST-06", {}),
...
WAVE2_EXPECTED_RUN_UNITS = 49_664
...
def expected_wave2_runkeys(config_root: Path) -> tuple[RunKeyV3, ...]:
    keys: list[RunKeyV3] = []
    for attack_id, filters in WAVE2_SPECS:
        path = config_root / f"{attack_id.lower()}.json"
        if not path.is_file():
            raise ValueError(f"missing Wave 2 config: {path.name}")
        config = json.loads(path.read_text(encoding="utf-8"))
        if config.get("attack_id") != attack_id:
            raise ValueError(f"Wave 2 config attack mismatch: {path.name}")
        freeze_id = config.get("freeze_id")
        if not isinstance(freeze_id, str) or not freeze_id:
            raise ValueError("Wave 2 config has invalid freeze_id")
...
def audit_and_merge_wave2(
    *,
    ...
    if execution.get("scope") not in ("internal", "full"):
        raise ValueError("Wave 2 requires internal/full execution scope")
    ...
    for path in record_paths:
        ...
        if run_key not in expected_ids:
            raise RuntimeError(f"unexpected Wave 2 RunKey: {run_key}")
        ...
        if record.get("phase") != "confirmatory":
            raise ValueError("Wave 2 record is not confirmatory")
        attack_id = record.get("attack_id")
        if attack_id not in required_attacks:
            raise ValueError("Wave 2 record has unexpected attack_id")
        if record.get("code_commit") != execution.get("source_commit"):
            raise ValueError("Wave 2 record source commit mismatch")
        if record.get("artifact_sha256") != execution.get("artifact_sha256"):
            raise ValueError("Wave 2 record artifact hash mismatch")
        if record.get("execution_manifest_sha256") != execution_sha256:
            raise ValueError("Wave 2 record execution-manifest hash mismatch")
        if record.get("preregistration_sha256") != execution.get("preregistration_sha256"):
            raise ValueError("Wave 2 record preregistration hash mismatch")
        if record.get("dependency_lock_sha256") != execution.get("dependency_lock_sha256"):
            raise ValueError("Wave 2 record dependency-lock hash mismatch")
        if record.get("config_sha256") != config_hashes[str(attack_id)]:
            raise ValueError("Wave 2 record config hash mismatch")
    ...
    (output_root / "wave2-ledger.json").write_bytes(canonical_json(ledger) + b"\n")
    summary = {
        "schema": "sigma-v3-r15-wave2-dataset-v1",
        "confirmatory": True,
    ...
    (output_root / "wave2-summary.json").write_bytes(canonical_json(summary) + b"\n")
    return summary


def main() -> int:
    parser = argparse.ArgumentParser(description="Audit and merge R15 internal Wave 2 shards.")
