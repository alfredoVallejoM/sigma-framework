"""Generate the byte-exact Sigma v3 R12 conformance corpus."""
...
DEFAULT_OUTPUT = ROOT / "specification" / "test-vectors" / "conformance-v3-r12.json"
MESSAGE = b"Sigma v3 R12 byte-exact corpus"
CHALLENGE = b"R12-conformance-challenge"
APPLICATION_CONTEXT = b"specification/test-vectors/conformance-v3-r12"
CASES = (
    ("wide-once-v3", 0x0301, b"R12-0301-50"),
    ("deep-v3", 0x0303, b"R12-0303-112"),
    ("deep-vector-v3", 0x0304, b"R12-0304-68"),
)
...
def render_corpus() -> str:
    document = {
        "schema": "sigma-v3-conformance-r12",
        "record_version": 3,
...
def main() -> int:
    ...
    if arguments.check:
        if not arguments.output.is_file() or arguments.output.read_text() != rendered:
            raise SystemExit("Sigma v3 R12 corpus is stale")
        return 0
